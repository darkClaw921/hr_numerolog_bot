...
async def is_premium(db_user: BotUser, subscription_repo: SubscriptionRepo | None = None) -> bool:
    ...
    Сначала проверяет список PREMIUM_USER_IDS (по telegram_id), затем — БД
    (подписка ключуется по внутреннему bot_users.id).
    """
...
async def require_premium(
    ...
    if isinstance(event, CallbackQuery):
        ...
        if event.message is not None:
            await event.message.answer(PAYWALL_TEXT, parse_mode="HTML")
    else:
        await event.answer(PAYWALL_TEXT, parse_mode="HTML")
    return False
